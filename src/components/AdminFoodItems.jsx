...
const AdminFoodItems = ({ setShow, setItem }) => {
    ...
    const handleChange = (e) => {
        ...
        const newList = food_list.filter((item) => item.name.toLowerCase().includes(value.toLowerCase()) || JSON.stringify(item.id).includes(value))
    ...
    return (
        <section className='flex flex-col gap-7 overflow-scroll scrollbar-hide'>
            ...
            {
                filteredList.length == 0 ? <p>No item found</p> : <table className="border-collapse w-full text-center text-sm md:text-lg">
                    <thead>
